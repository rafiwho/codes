...
using namespace std;
constexpr static int base1 = 121, base2 = 263, mod1 = 1E9 + 7, mod2 = 1E9 + 9;
constexpr static int N = 1E5 + 5; // modify
...
public:
  ...
  array<int, 2> get_substr_hash(int l, int r) {
      assert(l <= r);
      array<int, 2> hs{0, 0};
      hs[0] = pref[r][0];
...
void tcase() {
  ...
  auto ok = [&](int mid)->bool{
    ...
    for (int i = 0; i + mid - 1 < n; ++i) {
      st.insert(s1.get_substr_hash(i,i+mid-1));
    }
    for (int i = 0; i + mid - 1 < n; ++i) {
      auto h = s2.get_substr_hash(i,i+mid-1);
      if(st.find(h)!=st.end()){
        ans = b.substr(i,mid);
        return true;
      }
